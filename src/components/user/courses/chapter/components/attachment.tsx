...
const Attachment = ({ slug, initialData, refetch }: AttachmentProps) => {
  ...
  const [deleteAttachment, { isLoading }] = useDeleteChapterAttachemntMutation();
  const [updateAttachment] = usePostChapterAttachemntMutation();
  ...
  const onSubmit = async (value: { attachments?: File[] }) => {
    ...
    if (res?.data) {
      toast.success("Chapter attachments updated");
      refetch();
      setisEditing(false);
    } else if (res.error) {
      toast.error("Failed to update chapter attachments");
    }
  };
  const handleDelete = async () => {
    ...
    if (attachmentToDelete !== null && accessToken) {
      ...
      const res = await deleteAttachment({ id, accessToken, slug });
      refetch();
    }
  };
  const handleCancel = () => {
